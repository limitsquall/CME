...
lc = np.linspace(3.3, 3.7, 10) # we calculate the energy of the system for each of these lattice constants 
energies = []

for a in lc: # iterates over all lattice constants
    structure = FaceCenteredCubic(latticeconstant=a, symbol='Ni', pbc=True, size=(7,7,7)) # creates a fcc crystal
    ...
    if a == lc[0]:
        write('Ni.xyz', structure, append=False) # writes the first configuration to the trajectory file
    else:
        write('Ni.xyz', structure, append=True) # appends all other configurations to trajectory file
...
plt.show()
np.savetxt('Ni.txt', np.c_[lc, energies], header='# lattice constant, energy per atom') # saves the calculated data in a txt file.
